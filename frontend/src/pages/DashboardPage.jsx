          ...
          {result && (
            isNonJob ? (
              ...
              <div className="glass-panel p-6 sm:p-7 rounded-3xl border border-indigo-500/30 bg-slate-900/80 space-y-6 animate-fade-in shadow-2xl">
                ...
                {/* REPORT HEADER BAR (FULL WIDTH TITLE, METADATA & CONTROL TOOLBAR) */}
                <div className="p-5 sm:p-6 rounded-3xl bg-slate-950/95 border border-indigo-500/30 shadow-2xl space-y-4 overflow-hidden">
                  {/* Top Row: Icon + Full Width Title + Badge + Metadata */}
                  <div className="flex items-start space-x-3.5 sm:space-x-4 w-full">
                    <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-2xl bg-gradient-to-tr from-indigo-600 via-sky-500 to-emerald-400 p-0.5 shadow-lg shadow-indigo-500/25 shrink-0 mt-0.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="px-2.5 py-0.5 rounded-full text-[10px] sm:text-[11px] font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 shrink-0 whitespace-nowrap shadow-sm">
                          {t('dashboard.verified_badge', 'VERIFIED')}
                        </span>
                      </div>
                      
                      {/* Metadata Subtitle - Positioned directly under Title */}
                      <div className="text-[11px] sm:text-xs text-slate-400 flex flex-wrap items-center gap-x-3 gap-y-1 font-medium mt-2">
                        <span className="text-slate-300 flex items-center gap-1.5">
                          <span>📅</span>
                          <span>{t('dashboard.analyzed_on', 'Analyzed on')}</span>
                          <strong className="text-slate-200 font-mono">{new Date(result.created_at || Date.now()).toLocaleString()}</strong>
                        </span>
                        <span className="text-slate-600 hidden sm:inline">•</span>
                        <span className="flex items-center gap-1.5">
                          <span>👤</span>
                          <span>{t('dashboard.target_user', 'Target')}:</span>
                          <strong className="text-sky-300 font-semibold">{user?.full_name || 'Student'}</strong>
                        </span>
                      </div>
                    </div>
                  </div>

                  {/* Bottom Row: Full-width responsive action controls toolbar */}
                  <div className="pt-3.5 border-t border-slate-800/80 flex flex-wrap items-center justify-start sm:justify-end gap-2.5 w-full">
                    {/* Language Selector */}
                    <div className="flex items-center space-x-1.5 bg-slate-900 border border-slate-800 hover:border-slate-700 rounded-xl px-2.5 sm:px-3 py-2 text-xs text-slate-200 shadow-sm transition">
                    ...
                    <button
                      onClick={handleResetScan}
                      className="flex items-center space-x-1.5 px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 hover:text-white text-xs font-semibold transition shadow-sm whitespace-nowrap"
                    ...
                    <button
                      ...
                      className="flex items-center justify-center space-x-1.5 px-3.5 py-2 rounded-xl btn-primary font-bold text-xs shadow-md transition hover:scale-[1.02] whitespace-nowrap"
                    >
